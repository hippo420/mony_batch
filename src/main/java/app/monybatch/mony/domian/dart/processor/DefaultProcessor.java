...
import app.monybatch.mony.domian.dart.dto.raw.DartDefaultRawDto;
import app.monybatch.mony.domian.dart.entity.DartDefault;
...
@Slf4j
@Component
public class DefaultProcessor extends BaseDisclosureProcessor implements DisclosureProcessor {

    @Override
    public DisclosureType getSupportedType() { return DisclosureType.DEFAULT; }

    @Override
    public List<DartDisclosureBase> process(JSONObject response, DartRssQueueDto queueItem) {
        List<DartDefaultRawDto> list = JsonUtil.convert(
                new org.json.JSONObject(response.toJSONString()), "list", DartDefaultRawDto.class);
    ...
    private DartDefault toEntity(DartDefaultRawDto r, DartRssQueueDto q) {
        DartDefault e = new DartDefault();
        setBaseFields(e, r.getRceptNo(), r.getCorpCode(), r.getCorpName(), r.getCorpCls(), q, getSupportedType());
        e.setDefaultAmount(r.getDfAmt());
        e.setDefaultBank(r.getDfBnk());
        e.setDefaultReason(truncate(r.getDfRs(), 500));
        e.setDefaultDt(r.getDfd());
        return e;
    }
}
